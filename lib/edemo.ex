defmodule Edemo do
  ...
  def parse_fn(<<char, rest::bitstring>>, true)
      when (char != 32 or char != 44) and (char >= 65 or char == 61 or char == 34),
      do: {:ok, :keypair, <<char>>, rest}
    ...
    case parse_fn(input, parsed.ts_set) do
      {:ok, :timestamp, charlist, rest} ->
        parse_wrap(
          ...
          Map.merge(
            parsed,
            %{timestamp: parsed.timestamp ++ charlist}
          ),
          set
        )
      ...
      {:ok, :keypair, charlist, rest} ->
        parse_wrap(
          ...
          Map.merge(
            parsed,
            %{key_buffer: parsed.key_buffer <> charlist}
          ),
          set
        )
  ...
  def process_keypair_buffer(parsed) do
    ...
    case String.length(key_buffer) do
      ...
      _ ->
        ...
        Map.merge(
          clear_parsed,
          %{attr: Map.merge(clear_parsed.attr, %{key => value})}
        )
    end
  end
